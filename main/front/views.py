...
def add_to_cart(request, product_id):
    active_cart = models.Cart.objects.filter(user=request.user, is_active=True).first()
    
    if active_cart:
        active_cart.products.add(product_id)
    else:
        active_cart = models.Cart.objects.create(user=request.user, is_active=True)
        active_cart.products.add(product_id)
